...
class KeyvalMakeError
{
    public static function call(?KeyvalContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new KeyvalContext([], null);
        }
        $op = $ctx->op ?? new KeyvalOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new KeyvalResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof KeyvalError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new KeyvalError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof KeyvalError) {
