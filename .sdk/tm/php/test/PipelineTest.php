...
{
    private static function utility(): KeyvalUtility
    {
        return new KeyvalUtility();
    }
    ...
    private static function ctx(array $over = []): KeyvalContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new KeyvalContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new KeyvalOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof KeyvalError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new KeyvalOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = KeyvalMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new KeyvalOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = KeyvalMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new KeyvalOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = KeyvalMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KeyvalSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = KeyvalMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new KeyvalResponse([]),
            'result' => new KeyvalResult([]),
        ]);
        [, $err] = KeyvalMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new KeyvalSpec(['step' => 's']),
            'response' => new KeyvalResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new KeyvalResult([]),
        ]);
        [, $err] = KeyvalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new KeyvalSpec(['step' => 's']),
            'response' => new KeyvalResponse(self::resp(200, ['v' => 1])),
            'result' => new KeyvalResult([]),
        ]);
        [, $err] = KeyvalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new KeyvalSpec(['step' => 's']),
            'response' => new KeyvalResponse(self::resp(200, ['v' => 2])),
            'result' => new KeyvalResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        KeyvalMakeResponse::call($ctx);
    ...
    {
        $preset = new KeyvalResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new KeyvalSpec([]),
            'response' => new KeyvalResponse([]),
            'result' => new KeyvalResult([]),
        ...
        [$got, $err] = KeyvalMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new KeyvalResult([])]);
        [, $err] = KeyvalMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new KeyvalSpec([]), 'result' => null]);
        [, $err] = KeyvalMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KeyvalSpec(['step' => 's']),
            'result' => new KeyvalResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new KeyvalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KeyvalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new KeyvalSpec(['step' => 's']),
            'result' => new KeyvalResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new KeyvalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = KeyvalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new KeyvalResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new KeyvalSpec([]),
            'result' => new KeyvalResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = KeyvalMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KeyvalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new KeyvalError('boom', 'boom');
        $utility->fetcher = function (KeyvalContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KeyvalSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = KeyvalMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (KeyvalContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = KeyvalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(KeyvalResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (KeyvalContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new KeyvalSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = KeyvalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new KeyvalResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new KeyvalSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = KeyvalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = KeyvalMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = KeyvalMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new KeyvalResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, KeyvalDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new KeyvalResult(['ok' => false])]);
        $this->expectException(KeyvalError::class);
        KeyvalDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new KeyvalResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', KeyvalMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new KeyvalBaseFeature();
        $b = new KeyvalBaseFeature();
        KeyvalFeatureAdd::call($ctx, $a);
        KeyvalFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): KeyvalBaseFeature
    {
        $f = new KeyvalBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        KeyvalFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): KeyvalContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new KeyvalSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = KeyvalPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new KeyvalResponse(['headers' => null]),
            'result' => new KeyvalResult([]),
        ]);
        KeyvalResultHeaders::call($ctx);
