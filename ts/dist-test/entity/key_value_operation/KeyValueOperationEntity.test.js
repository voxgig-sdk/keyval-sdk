...
(0, node_test_1.describe)('KeyValueOperationEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when KEYVAL_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('KEYVAL_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.KeyvalSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if (!live && (0, utility_1.maybeSkipControl)(t, 'entityOp', 'key_value_operation.' + op, live))
                return;
        }
        ...
        let key_value_operation_ref01_data = Object.values(setup.data.existing.key_value_operation)[0];
        // LOAD
        const key_value_operation_ref01_ent = client.KeyValueOperation();
        const key_value_operation_ref01_match_dt0 = {};
        ...
        (0, node_assert_1.default)(key_value_operation_ref01_data_dt0.id === key_value_operation_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/key_value_operation/KeyValueOperationTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.KeyvalSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['key_value_operation01', 'key_value_operation02', 'key_value_operation03', 'get01', 'get02', 'get03', 'set01', 'set02', 'set03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        ...
        client = new __1.KeyvalSDK(merge([
...
//# sourceMappingURL=KeyValueOperationEntity.test.js.map