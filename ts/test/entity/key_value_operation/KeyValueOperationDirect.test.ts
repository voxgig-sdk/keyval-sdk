...
describe('KeyValueOperationDirect', async () => {
  ...
  test('direct-load-key_value_operation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-key_value_operation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['KEYVAL_TEST_KEY_VALUE_OPERATION_ENTID']
