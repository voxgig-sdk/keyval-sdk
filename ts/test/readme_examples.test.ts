...
//     (\`new KeyvalSDK(...)\` / \`KeyvalSDK.test(...)\` become
//     \`KeyvalSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"key_value_operation":{"test01":{"id":"test01"}},"nt":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new KeyvalSDK(...)\` and any
// \`KeyvalSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+KeyvalSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/KeyvalSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
