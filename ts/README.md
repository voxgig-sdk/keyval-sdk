# Keyval TypeScript SDK



The TypeScript SDK for the Keyval API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const keyvalueoperation = await client.KeyValueOperation().load({ id: 'example_id' })
...
| `tester(testopts?, sdkopts?)` | `KeyvalSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `KeyvalSDK.test(testopts?, sdkopts?)` | `KeyvalSDK` | Create a test-mode client. |
...
| `client` | `client(): KeyvalSDK` | Return the parent SDK client. |
