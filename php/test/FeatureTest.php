...
class FtCtrl extends KeyvalControl
{
...
class FtClient
{
    ...
    private KeyvalUtility $utility;

    public function __construct(KeyvalUtility $utility, array $options)
    {
    ...
    public function get_utility(): KeyvalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KeyvalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KeyvalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KeyvalConfig::make_feature($name);
    ...
    private static function buildUrl(KeyvalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KeyvalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KeyvalResponse($fetched);
        }
    ...
    private function fail(KeyvalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KeyvalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
