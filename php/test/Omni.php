...
//    only - a typed KeyvalContext there would make every ctx
...
final class KeyvalOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KeyvalContext($clean, null);
    ...
    public static function err_from_map($m): ?KeyvalError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KeyvalError(is_string($code) ? $code : '', $msg);
    }
