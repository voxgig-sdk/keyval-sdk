...
The PHP SDK for the Keyval API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->KeyValueOperation()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
try {
    // load() returns the ENTITY — call data_get() for the KeyValueOperation record (throws on error).
    $keyvalueoperation = $client->KeyValueOperation()->load(["key" => "example_key"]);
    print_r($keyvalueoperation->data_get());
} catch (\Throwable $err) {
...
```php
$client = KeyvalSDK::test($testopts, $sdkopts);
```
...
```php
// load() returns the ENTITY — call data_get() for the Nt record (throws on error).
...
The main class (`keyval_sdk.php`) exports the SDK class
