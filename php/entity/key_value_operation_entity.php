...
class KeyValueOperationEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "key_value_operation";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new KeyValueOperationEntity($this->_client, $opts);
    }

    /**
     * @param KeyValueOperation|array $args KeyValueOperation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = KeyvalHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return KeyValueOperation|array The current KeyValueOperation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of KeyValueOperation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = KeyvalHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of KeyValueOperation fields).
     */
    ...
    /**
     * Load a single KeyValueOperation.
     *
     * @param KeyValueOperationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed KeyValueOperationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return KeyValueOperation|array The loaded KeyValueOperation as an assoc-array at the
     *   SDK boundary; throws KeyvalError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = KeyvalHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
