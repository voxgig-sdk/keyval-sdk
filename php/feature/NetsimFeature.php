...
class KeyvalNetsimFeature extends KeyvalBaseFeature
{
    ...
    private function _simulate(KeyvalContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(KeyvalContext $ctx, array $applied): void
    {
        $client = $this->client;
