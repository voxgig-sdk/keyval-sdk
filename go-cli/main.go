// Command Keyval-cli is a CLI + REPL for the Keyval SDK,
...
const entitiesHelp = "key_value_operation nt"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      KEYVAL_APIKEY (api key), KEYVAL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: KEYVAL_APIKEY carries the API key and
	// KEYVAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("KEYVAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("KEYVAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewKeyvalSDK(opts)
