...
boru-driven command-line client **and** interactive REPL for the Keyval
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/keyval-cli)
make build
...
./keyval-cli load 1 key_value_operation            # {id:1} shorthand
./keyval-cli load '{id:1}' key_value_operation       # explicit match map

# 5. Override the API base URL for a single call
KEYVAL_BASE=https://api.example.com ./keyval-cli load 1 key_value_operation

# 6. No arguments -> interactive REPL
./keyval-cli
keyval> load 1 key_value_operation
keyval> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/keyval-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/keyval-cli load 1 key_value_operation
   ```
...
```sh
./keyval-cli load 1 key_value_operation          # scalar shorthand for {id:1}
./keyval-cli load '{id:1}' key_value_operation     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `keyval>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `key_value_operation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/keyval-cli`. |
...
(`./keyval-cli <expr>`) and interactively (the REPL), and expressions compose the
