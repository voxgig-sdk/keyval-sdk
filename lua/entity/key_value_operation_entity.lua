...
local KeyValueOperationEntity = {}
KeyValueOperationEntity.__index = KeyValueOperationEntity


function KeyValueOperationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, KeyValueOperationEntity)
  self._name = "key_value_operation"
  self._client = client
...
function KeyValueOperationEntity:get_name()
  return self._name
end


function KeyValueOperationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return KeyValueOperationEntity.new(self._client, opts)
end
...
function KeyValueOperationEntity:mark_deleted()
  self._deleted = true
end


function KeyValueOperationEntity:deleted()
  return true == self._deleted
end


function KeyValueOperationEntity:data_set(args)
  if args ~= nil then
...
function KeyValueOperationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function KeyValueOperationEntity:match_set(args)
  if args ~= nil then
...
function KeyValueOperationEntity:match_get()
...
function KeyValueOperationEntity:stream(action, args, callopts)
...
---@param reqmatch KeyValueOperationLoadMatch
---@param ctrl? table
---@return KeyValueOperation
---@return string? err
function KeyValueOperationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function KeyValueOperationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return KeyValueOperationEntity
