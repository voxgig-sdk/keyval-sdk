...
local NtEntity = {}
NtEntity.__index = NtEntity


function NtEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NtEntity)
  self._name = "nt"
  self._client = client
...
function NtEntity:get_name()
  return self._name
end


function NtEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NtEntity.new(self._client, opts)
end
...
function NtEntity:mark_deleted()
  self._deleted = true
end


function NtEntity:deleted()
  return true == self._deleted
end


function NtEntity:data_set(args)
  if args ~= nil then
...
function NtEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NtEntity:match_set(args)
  if args ~= nil then
...
function NtEntity:stream(action, args, callopts)
...
---@param reqmatch NtLoadMatch
---@param ctrl? table
---@return Nt
---@return string? err
function NtEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function NtEntity:_run_op(ctx, post_done)
  local utility = self._utility
