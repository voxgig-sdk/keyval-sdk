...
KeyvalSDK._make_feature = _make_feature


function KeyvalSDK.new(options)
  local self = setmetatable({}, KeyvalSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function KeyvalSDK:options_map()
...
function KeyvalSDK:get_utility()
  return Utility.copy(self._utility)
end


function KeyvalSDK:get_root_ctx()
  return self._rootctx
end


function KeyvalSDK:prepare(fetchargs)
...
function KeyvalSDK:direct(fetchargs)
...
-- by passing it.
function KeyvalSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function KeyvalSDK:graphql(query, variables, ctrl)
...
function KeyvalSDK:KeyValueOperation(data)
  ...
  if data == nil then
    if self._key_value_operation == nil then
      self._key_value_operation = EntityMod.new(self, nil)
    end
    return self._key_value_operation
  end
...
function KeyvalSDK:Nt(data)
  ...
  if data == nil then
    if self._nt == nil then
      self._nt = EntityMod.new(self, nil)
    end
    return self._nt
  end
  return EntityMod.new(self, data)
...
function KeyvalSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = KeyvalSDK.new(sdkopts)
  sdk.mode = "test"
