# Keyval Lua SDK Reference

Complete API reference for the Keyval Lua SDK.
...
```lua
local result, err = client:KeyValueOperation(nil):load({ id = "key_value_operation_id" }, nil)
```
...
```lua
local result, err = client:Nt(nil):load({ id = "nt_id" }, nil)
```
