...
import (
	...
	omni "github.com/voxgig-sdk/keyval-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KeyvalError); is {
				out := map[string]any{
					"name":    "KeyvalError",
...
func omniCtx(arg any, client *sdk.KeyvalSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KeyvalSDK); is {
			client = live
