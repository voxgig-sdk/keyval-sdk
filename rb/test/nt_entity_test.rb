...
class NtEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = nt_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "nt." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KEYVAL_TEST_NT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def nt_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "nt", "NtTestData.json")
  ...
  entid_env_raw = ENV["KEYVAL_TEST_NT_ENTID"]
