...
class KeyValueOperationEntity
  ...
  def make
    opts = @_entopts.dup
    KeyValueOperationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = KeyvalHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [KeyValueOperation, Hash] the current KeyValueOperation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = KeyvalHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of KeyValueOperation fields)
  def match_get
  ...
  #
  # @param reqmatch [KeyValueOperationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.KeyValueOperation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [KeyValueOperation, Hash] the loaded KeyValueOperation; raises KeyvalError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = KeyvalHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
