...
[MCP](https://modelcontextprotocol.io) server exposing the Keyval SDK as
two agent tools — `keyval_list` and `keyval_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/keyval-mcp)
make build

# 2. Provide credentials via the environment
export KEYVAL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user keyval \
  -- /absolute/path/to/keyval-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./keyval-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// keyval_list: first page of records
{ "entity": "key_value_operation" }
{ "entity": "key_value_operation", "query": { } }

// keyval_load: one record by id
{ "entity": "key_value_operation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/keyval-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user keyval \
     -- "$PWD"/dist/*/keyval-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `keyval_list` and `keyval_load` tools now appear
   in new sessions. Ask the agent to *"list key_value_operation using keyval"*
   and it calls `keyval_list` with `{"entity":"key_value_operation"}`.
...
```sh
export KEYVAL_APIKEY=sk_live_xxx            # API key
export KEYVAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `keyval_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `keyval_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KEYVAL_APIKEY` | API key sent with every request. |
| `KEYVAL_BASE` | Optional override of the API base URL. |
...
```sh
./keyval-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"keyval_load","arguments":{"entity":"key_value_operation","query":{"id":1}}}}'
```
