...
MCP server exposing the Keyval SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `keyval_list` | `entity`, optional `query` map | First page of records as JSON |
| `keyval_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `keyval_list` and
`keyval_load` tools become available in new sessions.
